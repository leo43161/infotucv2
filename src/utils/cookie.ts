...
export function encriptar(string: string) {
...
export function desencriptar(encryptedString: string) {
    try {
        const [ivHex, encryptedBase64] = encryptedString.split(':');
        const iv = CryptoJS.enc.Hex.parse(ivHex);
        const ciphertext = CryptoJS.enc.Base64.parse(encryptedBase64);
        const ciphertextParams = CryptoJS.lib.CipherParams.create({ ciphertext: ciphertext });
        const decrypted = CryptoJS.AES.decrypt(ciphertextParams, key, { iv: iv });
