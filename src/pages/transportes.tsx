      ...
      >
        ...
        {infoDestino && infoDestino?.length > 0 && (
          infoDestino.map((colectivo: Colectivo) => (
            <div
              key={colectivo.IdAutobus}
              className={clsx(
                'z-10 h-full shrink-0 overflow-hidden rounded-xl mx-auto',
                infoDestino?.length > 3 ? 'w-4/12' : infoDestino?.length === 1 ? 'w-8/12' : 'flex-1'
              )}
            >
        ...
        <div className='flex flex-wrap '>
          {destinos.map((destino, index) => {
            ...
            return (
              ...
              >
                <h4 className={`font-bold text-zinc-100 ${verMas ? 'text-3xl' : 'text-[2.1em]'} z-10`}>
                  {destino.nombre}
                </h4>
              </div>
            )
          ...
          <div
            className='bg-primary py-2 px-5 border grow flex-1/5 shrink-0 flex justify-center items-center'
            onClick={() => setVerMas(!verMas)}
          >
            <h4 className='font-bold text-zinc-100 text-4xl underline'>
              {verMas ? t("transportation.see_less") : t("transportation.see_more")}
            </h4>
          </div>
        </div>
          ...
          {circuitos.filter((_, index) => index !== 0).map((circuito) => (
            <div key={circuito.name} className='flex-1 relative'>
              <button
                ...
                onClick={() =>
                  circuito === circuitoActivo && verMas ? setVerMas(false) : setCircuitoActivo(circuito)
                }
              >
                <img
